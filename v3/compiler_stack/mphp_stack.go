	...
	statementProc = map[string]func(*Statement, *Block) error{
		...
		"access": func(statement *Statement, context *Block) error {
			...
			case "var":
				...
				if context.isNotLocal(symName) {
					return errors.New("Handling of globals not implemented")
				}

				command := NewCommand("retrieve", context.getSymbolPosition(symName), 0)
			...
			case "array":
				...
				command := NewCommand("retrievei", context.getSymbolPosition(symName), 0)
		...
		"assign": func(statement *Statement, context *Block) error {
			...
			case "var":
				...
				symName := statement.children[0].children[0].command
				if context.isNotLocal(symName) {
					return errors.New("Cannot handle globals")
				}
				...
				cmd := NewCommand("store", context.getSymbolPosition(symName), 0)
			...
			case "array":
				...
				cmd := NewCommand("storei", context.getSymbolPosition(symName), 0)
		...
		"func": func(statement *Statement, context *Block) error {
			...
			AddCommand(NewCommand("funcexit", funcBlock.nextSymPos, 0))

			/* jetzt noch die symbols vom entrycommand setzen */
			entryCommand.p1 = int64(funcBlock.nextSymPos)
			/* in p2 wird die anzahl der lokalen variablen gespeichert */
			entryCommand.p2 = funcBlock.nextSymPos - int64((len(statement.children) - 1))
		...
		"return": func(statement *Statement, context *Block) error {
			...
			// Adresse des literals ist jetzt am stack
			// diese wird jetzt in variable gespeichert
			//cmd := NewCommand("store", context.getSymbolPosition(returnValueSymbolName), 0 /* retval ist immer ein local */)
			...
			AddCommand(NewCommand("return", 0, 0))
...
func ProcessForeach(foreachStatement *Statement, parent *Block) error {
	...
	// diese wird jetzt in variable gespeichert
	cmd := NewCommand("store", foreachBlock.getSymbolPosition(intSym), 0)
	...
	cmd = NewCommand("retrieve", foreachBlock.getSymbolPosition(intSym), 0)
	cmd.resolvingHint = intSym
	AddCommand(cmd)

	if foreachBlock.isNotLocal(arrSym) {
		return errors.New("Cannot use global array in foreach")
	}

	/* 05 - länge des arrays holen */
	cmd = NewCommand("len", foreachBlock.getSymbolPosition(arrSym), 0)
	...
	cmd = NewCommand("retrievei", foreachBlock.getSymbolPosition(arrSym), 0)
	cmd.resolvingHint = arrSym
	AddCommand(cmd)

	/* 10 - ergebnis in val speichern */
	cmd = NewCommand("store", foreachBlock.getSymbolPosition(valSym), 0)
	...
	//AddCommand(NewCommand("blockexit", int64(foreachBlock.nextSymPos), 0))

	/* jetzt noch die symbols vom entrycommand setzen */
	//entryCommand.p1 = int64(foreachBlock.nextSymPos)
...
func ProcessFor(forStatement *Statement, parent *Block) error {
	...
	*/
	...
	/* blockexit */
	/* hier weiss ich schon wieviele symbols der block hat */
	//AddCommand(NewCommand("blockexit", int64(forBlock.nextSymPos), 0))

	/* jetzt noch die symbols vom entrycommand setzen */
	//entryCommand.p1 = int64(forBlock.nextSymPos)
...
func ProcessWhile(whileStatement *Statement, parent *Block) error {
	...
	/* blockexit */
	/* hier weiss ich schon wieviele symbols der block hat */
	//AddCommand(NewCommand("blockexit", int64(whileBlock.nextSymPos), 0))

	/* jetzt noch die symbols vom entrycommand setzen */
	//entryCommand.p1 = int64(whileBlock.nextSymPos)
...
func (block *Block) Process(parent *Block) error {
	...
	*/
	...
	if block.parent == nil {
		AddCommand(NewCommand("progexit", int64(block.nextSymPos), 0))
	} else {
		//AddCommand(NewCommand("blockexit", int64(block.nextSymPos), 0))
	}

	if entryCommand != nil {
		entryCommand.p1 = int64(block.nextSymPos)
	}
