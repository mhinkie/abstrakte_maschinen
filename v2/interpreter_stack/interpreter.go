...
var commandImpl = []func() (newCommandPointer int, err error){
	...
	/* 7 - blockentry */
	func() (int, error) {
		fmt.Fprintf(os.Stderr, "ACTHUNG BLOCKENTRY CALLED (IRGENDWAS MACHT DER COMPILER FALSCH)")
	...
	func() (int, error) {
		...
		// Anzahl an aufruf parametern
		anzParam := P2(commandPointer)

		// Die aufrufparameter werden jetzt um 3 stellen am stack vorverschoben (damit in den freiwerdenden feldern die aufrufinfos und returninfos stehen können)
		// Beginne beim letzten parameter
		for i := int64(0); i < anzParam; i++ {
			stackPointer--
			stack[stackPointer+3] = stack[stackPointer]
		}

		// Stackpointer steht jetzt auf zeile für retv
		...
		stack[stackPointer] = int64(commandPointer + commandSize) /* es wird +1 gerechnet, damit er beim return nur übernommen werden muss */
		stackPointer++

		// Jetzt den stackpointer wieder um die die anzahl der variablen erhöhen, damit er wieder nach der letzten variable steht
		stackPointer += int(anzParam)
	...
	func() (int, error) {
		...
		stackPointer = stackPointer + int(P2(commandPointer))
		framePointer = stackPointer - int(P1(commandPointer))

		return Advance(commandPointer), nil
	},
	/* 28 - funcexit */
	funcExitImplementation,
	/* 29 - blockreturn */
	func() (int, error) {
	...
	func() (int, error) {
		...
		/* schreibt in die return cell und ruft dann funcexit auf */

		/* return value steht 3 vor framepointer */
		stackPointer--
		stack[framePointer-3] = stack[stackPointer]

		return funcExitImplementation()
	},
}

func funcExitImplementation() (int, error) {
	/* function-exit */

	// programstatus davor:
	// stack: ...|retv|ofp|orip|[FP]x|y|z| <- sp
	// rip = xxx
	// danach:
	// stack: |[FP]| | ...|retv| <- SP
	// rip = rip
	...
	framePointer = int(stack[stackPointer]) /* wert nach retval = framePointer */

	retAddress := stack[stackPointer+1]

	// Jetzt kann gejumped werden
	return JumpAbsolute(int(retAddress)), nil
}
