...
/* Liefert den nächst drüberen function block */
func (block *Block) FunctionBlock() *Block {
	if block.functionBlock {
		return block
	} else {
		return block.parent.FunctionBlock()
	}
}
...
func (block *Block) isNotLocal(name string) bool {
	if !block.isSymbolDefined(name) {
		return false
	}
	return !(block.getDefiningBlock(name).FunctionBlock() == block.FunctionBlock())
}

func (block *Block) getDefiningBlock(name string) *Block {
	_, ok := block.symbols[name]
	if ok {
		return block
	} else {
		return block.parent.getDefiningBlock(name)
	}
}
...
func (block *Block) addSymbol(name string, symType PHPType) {
	if !block.isSymbolDefined(name) {
		...
		/* die symbol position wird im functionblock vergeben */
		symPos := block.FunctionBlock().nextSymPos
		block.FunctionBlock().nextSymPos += symOffset
		block.symbols[name] = symPos
...
func newEmptyBlock() Block {
	...
	var block = Block{make(map[string]int64), make(map[string]PHPType), []BlockContent{}, nil, 0, "", "", false}
...
func newBlock(statements []Statement) Block {
	...
	var block = Block{make(map[string]int64), make(map[string]PHPType), bContent, nil, 0, "", "", false}
