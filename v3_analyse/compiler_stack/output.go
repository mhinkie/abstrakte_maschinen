...
func outputInt64(value int64) {
	oByte := make([]byte, 8)
	binary.LittleEndian.PutUint64(oByte, uint64(value))
...
func (command Command) String() string {
	cCode, _ := getCommandName(command.commandCode)
	return "(" + cCode + ", " + strconv.Itoa(int(command.p1)) + ", " + strconv.Itoa(int(command.p2)) + ", " + strconv.FormatBool(command.unresolvedFunction) + ", " + strconv.FormatBool(command.unresolvedJump) + ", " + command.resolvingHint + ")"
}
